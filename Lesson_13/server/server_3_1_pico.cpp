...
class PicoParser {
public:
  void parseRequest(char* http, size_t nbytes);  // for request
};

void PicoParser::parseRequest(char* http, size_t nbytes) {
  const char* method, *path;
  size_t method_length, path_length, number_headers;
  int minor_version;
  phr_header headers[100];

  int nparsed = phr_parse_request(http, nbytes, &method, &method_length, &path, &path_length, &minor_version, headers, &number_headers, 0);
  if (nparsed == -1) {
    ERR("Parse error: %s", http);
    MSG("nbytes = %zu", nbytes);
    throw ParseException();
  } else if (nparsed == -2) {
    WRN("Request is incomplete, continue in loop...");
  } else {
    // OK parser
    INF("HTTP version: 1.%i", minor_version);
    INF("method: %.*s", (int) method_length, method);
    INF("path: %.*s", (int) path_length, path);
...
void Server::workerThread(int socket) {
  char buffer[BUFFER_LENGTH];
  int read_bytes = 0;
  while ((read_bytes = recv(socket, buffer, BUFFER_LENGTH, 0)) != 0) {
    // process data: here - simply print
    MSG("Received: %s", buffer);
    std::string text;

    try {
      m_parser.parseRequest(buffer, read_bytes);
      // send OK response
      text = "HTTP/1.1 200 OK\r\nContent-length: 56\r\nContent-Type: text/html\r\nConnection: Closed\r\n\r\n<html>\n\t<body>\n\t\t<h1>Hello, World!</h1>\n\t</body>\n</html>\r\n";
    } catch (ParseException exception) {
      WRN("Failed to parse incoming request");
      // send Error response
      text = "HTTP/1.1 500 Server error\r\nContent-length: 56\r\nContent-Type: text/html\r\nConnection: Closed\r\n\r\n<html>\n\t<body>\n\t\t<h1>Server error!</h1>\n\t</body>\n</html>\r\n";
    }
...
void Server::handleClientMessages(int data_transfer_socket) {
  std::thread t(&Server::workerThread, this, data_transfer_socket);
  t.detach();
}
...
int main(int argc, char** argv) {
  DBG("[Lesson 13]: Server 3.1.pico");

  Server server;
  server.run();

  DBG("[Lesson 13]: Server 3.1.pico END");
  return 0;
}
